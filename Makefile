...
init: ## Initialize Terraform
	...
	sg incus-admin -c 'cd terraform/nodes && terraform init'
	@echo "Initializing Bootstrap..."
	sg incus-admin -c 'cd terraform/bootstrap && terraform init'

plan: ## Plan Terraform changes
	@echo "Planning Nodes..."
	sg incus-admin -c 'cd terraform/nodes && terraform plan'
	@echo "Planning Bootstrap..."
	sg incus-admin -c 'cd terraform/bootstrap && terraform plan'

apply: ## Apply Terraform changes
	@echo "Applying Nodes..."
	sg incus-admin -c 'cd terraform/nodes && terraform apply -auto-approve'
	@echo "Applying Bootstrap..."
	sg incus-admin -c 'cd terraform/bootstrap && terraform apply -auto-approve'

destroy: ## Destroy Terraform infrastructure
	@echo "Destroying Bootstrap..."
	sg incus-admin -c 'cd terraform/bootstrap && terraform destroy -auto-approve'
	@echo "Destroying Nodes..."
	sg incus-admin -c 'cd terraform/nodes && terraform destroy -auto-approve'
...
status: ## Check cluster status
	...
	@echo "  sg incus-admin -c 'incus exec k8s-control-plane -- sudo -i -u ubuntu'"
...
ssh-control: ## SSH into control plane
	...
	sg incus-admin -c 'incus exec k8s-control-plane -- sudo -i -u ubuntu'

ssh-worker1: ## SSH into worker 1
	@echo "Connecting to worker 1..."
	sg incus-admin -c 'incus exec k8s-worker-1 -- sudo -i -u ubuntu'

ssh-worker2: ## SSH into worker 2
	@echo "Connecting to worker 2..."
	sg incus-admin -c 'incus exec k8s-worker-2 -- sudo -i -u ubuntu'
